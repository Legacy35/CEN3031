<?php
    ...
    if(isset($_GET['token'])){ //Has the client included a 'token' variable as part of the GET request?
          ...
          $statement = $conn->prepare("SELECT id, email, admin FROM authentication WHERE token = ?"); //Creates the fill in the blank SQL statement, but variables are not yet decided. Does not execute the statement until we call the execute function.
          ...
          if($resultSet->num_rows < 1) exit(json_encode(array('error' => 'User with provided token not found.'))); //Make sure we have more than 0 results
